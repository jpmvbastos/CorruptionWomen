...
foreach v in  $corrupt {
	...
	* PANEL A: OLS with Baseline Controls
	...
	foreach k in $share_outcomes {
		...
		eststo: reg share_`k'_`v' corruption3_full $controls $fixedef, cluster(uf_code)	
		
	}
	
	esttab using "Results/revision1/sectors/Table2A_`v'.tex", replace b(%9.3f) ///
	...
	* PANEL C: 2SLS Results with Baseline Controls 
	...
	foreach k in $share_outcomes {
		...
		* Run the 2SLS regression and store the results
		eststo: ivreg2 share_`k'_`v' $controls  _I* ///
	...
	* Export the results for all models in a single table
	esttab using "Results/revision1/sectors/Table2C_`v'.tex", replace b(%9.3f) keep(corruption3_full $controls) ///
	...
	* PANEL D: 2SLS Results with Baseline Controls and Industry Shares
	...
	foreach k in $share_outcomes {
		...
		* Run the 2SLS regression and store the results
		eststo: ivreg2 share_`k'_`v' $controls $sectors _I* ///
		(corruption3_full = councils councils_installed management judge), ///
		first cluster(uf_code) partial(_cons $fixedef)
	...
	* Export the results for all models in a single table
	esttab using "Results/revision1/sectors/Table2D_`v'.tex", replace b(%9.3f) ///
...
******************** TABLE 3 - PERCENT OF FEMALE LABOR FORCE *******************
	...
	* PANEL A: OLS with Baseline Controls
	...
	foreach k in  $pct_outcomes {
		...
		eststo: reg pct_`k'_`v' corruption3_full $controls $fixedef, cluster(uf_code)
	
	}

	esttab using "Results/revision1/sectors/Table3A_`v'.tex", replace b(%9.3f) ///
	keep(corruption3_full $controls) star(* 0.10 ** 0.05 *** 0.01) se ar2
	...
	* PANEL B: OLS with Baseline Controls plus Industry Shares
	...
	foreach k in  $pct_outcomes {
		...
		eststo: reg pct_`k'_`v' corruption3_full $controls $sectors $fixedef, cluster(uf_code)
	
	}

	esttab using "Results/revision1/sectors/Table3B_`v'.tex", replace b(%9.3f) ///
	keep(corruption3_full $controls) star(* 0.10 ** 0.05 *** 0.01) se ar2
	...
	* PANEL C: 2SLS Results with Baseline Controls 
	...
	foreach k in $pct_outcomes {
		...
		* Run the 2SLS regression and store the results
		eststo: ivreg2 pct_`k'_`v' $controls _I* ///
		(corruption3_full = councils councils_installed management judge), ///
		first cluster(uf_code) partial(_cons $fixedef)
	...
	* Export the results for all models in a single table
	esttab using "Results/revision1/sectors/Table3C_`v'.tex", replace b(%9.3f) keep(corruption3_full $controls) ///
	star(* 0.10 ** 0.05 *** 0.01) stats(N F_Statistic J_Statistic J_Statistic_pvalue) se
	...
	* PANEL D: 2SLS Results with Baseline Controls plus Industry Shares
	...
	foreach k in $pct_outcomes {
		...
		* Run the 2SLS regression and store the results
		eststo: ivreg2 pct_`k'_`v' $controls $sectors _I* ///
		(corruption3_full = councils councils_installed management judge), ///
		first cluster(uf_code) partial(_cons $fixedef)
	...
	* Export the results for all models in a single table
	esttab using "Results/revision1/sectors/Table3D_`v'.tex", replace b(%9.3f) ///
	keep(corruption3_full $controls) star(* 0.10 ** 0.05 *** 0.01) ///
	stats(N F_Statistic J_Statistic J_Statistic_pvalue) se 
